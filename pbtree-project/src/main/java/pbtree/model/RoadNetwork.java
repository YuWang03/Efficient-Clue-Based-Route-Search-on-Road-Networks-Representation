...
public class RoadNetwork {
    ...
    /**
     * Compute all distances from a source vertex.
    ...
    public void removeIsolatedNodes() {
        ...
        for (List<Edge> edges : adjacencyList.values()) {
            for (Edge e : edges) {
                connected.add(e.getFrom());
                connected.add(e.getTo());
            }
        }
        nodes.keySet().retainAll(connected);
    }
    ...
    public void printStatistics() {
        ...
        // Top keywords
        List<Map.Entry<String, Set<Long>>> sorted = new ArrayList<>(keywordIndex.entrySet());
        sorted.sort((a, b) -> Integer.compare(b.getValue().size(), a.getValue().size()));
        
        System.out.println("Top 10 keywords:");
        for (int i = 0; i < Math.min(10, sorted.size()); i++) {
            System.out.println("  " + sorted.get(i).getKey() + ": " + sorted.get(i).getValue().size() + " nodes");
        }
    }
}
